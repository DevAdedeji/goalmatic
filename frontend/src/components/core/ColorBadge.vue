<template>
	<div :style="`background-color:${bg || selectPalette?.bg}; color:${color || selectPalette?.color}; border-color: ${color || selectPalette?.color}; border-radius: ${radius};`"
		class="px-3 py-1 text-xs font-medium  inline-block border">
		<slot> {{ name ? translateStatus(name) : '' }}</slot>
	</div>
</template>
...
const selectPalette = computed(() => {
	if (!props.name) return palette.default
