<template>
	...
	<div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
		<div v-for="(agent, index) in [defaultGoalmaticAgent, ...fetchedAllAgents]" :key="agent?.id || index" class="flex flex-col card" @click="$router.push(`/agents/explore/${agent?.id || 0}`)">
