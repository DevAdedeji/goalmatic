		...
		<section v-if="true || !loading" class="">
			<div class="flex w-full justify-start">
				<Tabs
					:selected="selected"
					:tabs="tabViews"
					:icons="[ClipboardList, Flag, CheckSquare]"
					:counts="[stepCount, milestoneCount, todoCount]"
					@changed="updateTab($event)"
				/>
			</div>
...
import { ArrowRight, ClipboardList, Flag, CheckSquare } from 'lucide-vue-next'
...
// Tab counts
const stepCount = computed(() => goalDetails.value?.steps?.length || 0)
const milestoneCount = computed(() => goalDetails.value?.milestones?.length || 0)
const todoCount = computed(() => goalDetails.value?.todos?.length || 0)
...
initTabs(
	...
	[ClipboardList, Flag, CheckSquare],
	[stepCount, milestoneCount, todoCount]
)
