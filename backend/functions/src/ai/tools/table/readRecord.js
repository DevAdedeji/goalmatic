...
exports.TABLE_READ = exports.formatTime = exports.parseTime = void 0;
        ...
        if (params.filter && params.filter.length > 0) {
            filteredRecords = records.filter(record => {
                return params.filter.every(filterItem => {
                    ...
                    const field = tableData.fields.find((f) => f.name.toLowerCase() === filterItem.field.toLowerCase());
                    ...
                    switch (fieldType) {
                        ...
                        case 'time':
                            ...
                            if (compareValue instanceof Date && filterValue instanceof Date) {
                                ...
                                const compareTimeOnly = new Date(baseDate.setHours(compareValue.getHours(), compareValue.getMinutes(), compareValue.getSeconds()));
                                const filterTimeOnly = new Date(baseDate.setHours(filterValue.getHours(), filterValue.getMinutes(), filterValue.getSeconds()));
                    ...
                    switch (filterItem.operator) {
                        case 'equals':
                            if (fieldType === 'date') {
                                ...
                                return compareValue.getTime() === filterValue.getTime();
                            }
...
const readTableRecordTool = (0, ai_1.tool)({
    ...
    parameters: zod_1.z.object({
        filter: zod_1.z.array(zod_1.z.object({
            field: zod_1.z.string().describe("The field NAME to filter on (case-insensitive)"),
            operator: zod_1.z.enum(['equals', 'contains', 'greater_than', 'less_than', 'starts_with', 'ends_with'])
                .describe("The comparison operator to use"),
            value: zod_1.z.any().describe("The value to compare against")
        })).optional().describe("Optional filters to apply to the records"),
        limit: zod_1.z.number().optional().describe("Maximum number of records to return")
    }),
    execute: async (input) => {
        try {
