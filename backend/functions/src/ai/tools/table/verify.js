"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.verifyTableAccess = void 0;
const init_1 = require("../../../init");
const verifyTableAccess = async (uid, tableId) => {
    // If a specific table ID is provided, check if it exists and belongs to the user
    if (tableId) {
        const tableDoc = await init_1.goals_db.collection('tables').doc(tableId).get();
        if (!tableDoc.exists) {
            return { exists: false };
        }
        const tableData = tableDoc.data();
        // Check if the table belongs to the user
        if (tableData?.creator_id !== uid) {
            return { exists: false };
        }
        return { exists: true, tableData };
    }
    // Otherwise, just check if the user exists
    const userDoc = await init_1.goals_db.collection('users').doc(uid).get();
    return {
        exists: userDoc.exists
    };
};
exports.verifyTableAccess = verifyTableAccess;
